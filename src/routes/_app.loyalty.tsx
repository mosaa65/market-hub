...
function LoyaltyPage() {
  ...
  async function load() {
    const [{ data: cs }, { data: ts }] = await Promise.all([
      supabase
        .from("customers")
        .select("id,name,phone,loyalty_points,balance")
  ...
  return (
    <>
      <PageHeader
        ...
        subtitle={
          lang === "ar"
            ? "إدارة نقاط العملاء (ربح / استبدال)"
            : "Manage customer points (earn / redeem)"
        }
      ...
      <div className="mb-4 grid grid-cols-2 gap-3 md:grid-cols-3">
        <Card>
          <CardContent className="p-4">
            <div className="text-xs text-muted-foreground">
              {lang === "ar" ? "إجمالي النقاط الموزعة" : "Total points outstanding"}
            </div>
        ...
        <Card>
          <CardContent className="p-4">
            <div className="text-xs text-muted-foreground">
              {lang === "ar" ? "أعضاء فعّالون" : "Active members"}
            </div>
        ...
        <Card>
          <CardContent className="p-4">
            <div className="text-xs text-muted-foreground">
              {lang === "ar" ? "حركات حديثة" : "Recent transactions"}
            </div>
                ...
                {filtered.length === 0 ? (
                  ...
                  filtered.map((c) => (
                    <TableRow key={c.id}>
                      ...
                      <TableCell className="text-end font-mono font-semibold text-amber-500">
                        {Number(c.loyalty_points).toFixed(0)}
                      </TableCell>
                    </TableRow>
                  ))
                )}
            ...
            <div className="space-y-2 max-h-[500px] overflow-y-auto">
              {tx.length === 0 ? (
                <div className="text-sm text-muted-foreground">
                  {lang === "ar" ? "لا توجد حركات" : "No activity"}
                </div>
              ) : (
                tx.map((t) => {
...
function AdjustDialog({ customers, onSaved }: { customers: Customer[]; onSaved: () => void }) {
  ...
  async function save() {
    ...
    setOpen(false);
    setForm({ customer_id: "", points: 10, kind: "earn", note: "" });
    onSaved();
  }
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogTrigger asChild>
        <Button>
          ...
          {lang === "ar" ? "تعديل نقاط" : "Adjust points"}
        </Button>
      </DialogTrigger>
      <DialogContent className="max-w-md">
        <DialogHeader>
          <DialogTitle>{lang === "ar" ? "تعديل نقاط الولاء" : "Adjust loyalty points"}</DialogTitle>
        </DialogHeader>
        <div className="grid gap-3">
              ...
              <SelectContent>
                {customers.map((c) => (
                  <SelectItem key={c.id} value={c.id}>
                    {c.name} ({Number(c.loyalty_points).toFixed(0)})
                  </SelectItem>
                ))}
              </SelectContent>
            </Select>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="grid gap-1.5">
              <Label>{lang === "ar" ? "النوع" : "Kind"}</Label>
          ...
          <Textarea
            rows={2}
            placeholder={lang === "ar" ? "ملاحظات" : "Note"}
            value={form.note}
