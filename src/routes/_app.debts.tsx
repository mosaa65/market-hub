...
function DebtsPage() {
  ...
  function printStatement() {
    ...
    const invoiceRows = invoices
      .map(
        (i) =>
          `<tr><td>${esc(date(i.created_at))}</td><td dir="ltr">${esc(i.invoice_number)}</td><td>${esc(money(Number(i.total)))}</td><td>${esc(money(Number(i.paid)))}</td><td class="balance">${esc(money(Number(i.total) - Number(i.paid)))}</td></tr>`,
      )
      .join("");
    const paymentRows = payments
      .map(
        (p) =>
          `<tr><td>${esc(date(p.payment_date))}</td><td dir="ltr">${esc(p.sales_invoices?.invoice_number ?? (rtl ? "دفعة على الحساب" : "On account"))}</td><td>${esc(pmLabel(p.payment_method))}</td><td class="paid">${esc(money(Number(p.amount)))}</td></tr>`,
      )
      .join("");
    const popup = window.open("", "_blank", "noopener,noreferrer");
    if (!popup) return;
    popup.document
      .write(`<!doctype html><html lang="${rtl ? "ar" : "en"}" dir="${rtl ? "rtl" : "ltr"}"><head><meta charset="utf-8"><title>${rtl ? "كشف حساب" : "Account statement"} — ${esc(selected.name)}</title><style>
