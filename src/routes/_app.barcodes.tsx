...
const LAYOUTS: Record<
  LayoutKey,
  { cols: number; rows: number; label: string; labelAr: string; w: string; h: string }
> = {
  "a4-4x10": {
    cols: 4,
    rows: 10,
    label: "A4 · 40 labels (4×10)",
    labelAr: "A4 · 40 ملصق (4×10)",
    w: "45mm",
    h: "25mm",
  },
  "a4-3x8": {
    cols: 3,
    rows: 8,
    label: "A4 · 24 labels (3×8)",
    labelAr: "A4 · 24 ملصق (3×8)",
    w: "63mm",
    h: "35mm",
  },
  "a4-2x5": {
    cols: 2,
    rows: 5,
    label: "A4 · 10 labels (2×5)",
    labelAr: "A4 · 10 ملصق (2×5)",
    w: "95mm",
    h: "50mm",
  },
  "roll-50": {
    cols: 1,
    rows: 1,
    label: "Roll · 50×30mm single",
    labelAr: "لفة · 50×30مم منفرد",
    w: "50mm",
    h: "30mm",
  },
};
...
function BarcodesPage() {
  ...
  async function loadProducts() {
    const { data } = await supabase
      .from("products")
      .select("id,name,name_ar,sku,barcode,sale_price")
      .eq("is_active", true)
      .order("name")
      .limit(1000);
  ...
  async function assignScannedBarcode(scanned: string) {
    if (!product) {
      toast.error(lang === "ar" ? "اختر منتجاً أولاً" : "Select a product first");
      return;
    }
    const { error } = await supabase
      .from("products")
  ...
  return (
    <>
      <PageHeader
        ...
        subtitle={
          lang === "ar"
            ? "توليد وطباعة ملصقات الباركود بأحجام متعددة"
            : "Generate and print barcode labels in multiple sizes"
        }
        actions={
            ...
            <>
              <div className="rounded-xl border border-border bg-surface p-3">
                <div className="text-[11px] text-muted-foreground mb-1">
                  {lang === "ar" ? "الباركود الحالي" : "Current barcode"}
                </div>
                <div className="font-mono text-sm">
                  {product.barcode || (
                    <span className="text-muted-foreground italic">
                      {lang === "ar" ? "لا يوجد" : "not set"}
                    </span>
                  )}
                </div>
                <div className="mt-2 flex gap-2">
                  <button
                    onClick={assignRandomBarcode}
                    className="flex-1 flex items-center justify-center gap-1 rounded-full border border-border bg-surface-2 px-3 py-1.5 text-xs hover:bg-surface"
                  >
                    <Wand2 className="h-3 w-3" />{" "}
                    {lang === "ar" ? "توليد EAN-13" : "Generate EAN-13"}
                  </button>
                </div>
              </div>
              ...
              <div className="space-y-2 rounded-xl border border-border p-3">
                <ToggleRow
                  label={lang === "ar" ? "إظهار اسم المنتج" : "Show product name"}
                  value={showName}
                  onChange={setShowName}
                />
                <ToggleRow
                  label={lang === "ar" ? "إظهار السعر" : "Show price"}
                  value={showPrice}
                  onChange={setShowPrice}
                />
                <ToggleRow
                  label={lang === "ar" ? "إظهار اسم المتجر" : "Show store name"}
                  value={showCompany}
          ...
          {!product ? (
            <div className="grid place-items-center py-24 text-center text-muted-foreground">
              ...
              <div className="text-sm">
                {lang === "ar" ? "اختر منتجاً لبدء المعاينة" : "Select a product to preview labels"}
              </div>
              <div className="mt-1 text-xs">
                {lang === "ar"
                  ? "أو استخدم زر الكاميرا لقراءة باركود موجود"
                  : "Or scan an existing barcode with the camera"}
              </div>
            </div>
          ) : (
            <div className="space-y-3">
              <div className="flex items-center justify-between text-xs text-muted-foreground">
                <span>
                  {lang === "ar" ? "المعاينة" : "Preview"} · {L.w} × {L.h}
                </span>
                ...
                >
                  {Array.from({ length: copies }).map((_, i) => (
                    ...
                    >
                      ...
                      <svg data-barcode />
                      {showPrice && (
                        <div className="pr" style={{ fontSize: 11, fontWeight: 700, marginTop: 1 }}>
        ...
        onDetected={(scanned) => {
          ...
          if (product) {
            ...
            if (match) {
              ...
              toast.success(lang === "ar" ? "تم اختيار المنتج" : "Product selected");
            } else {
              setSearch(scanned);
              toast(
                lang === "ar"
                  ? `الباركود ${scanned} غير مرتبط`
                  : `${scanned} not linked to any product`,
              );
            }
          }
        }}
