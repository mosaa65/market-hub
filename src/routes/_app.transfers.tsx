...
function TransfersPage() {
  ...
  async function load() {
    const { data } = await supabase
      ...
      .select(
        "*, from:warehouses!stock_transfers_from_warehouse_id_fkey(name,name_ar), to:warehouses!stock_transfers_to_warehouse_id_fkey(name,name_ar), stock_transfer_items(quantity)",
      )
              ...
              {list.length === 0 ? (
                <TableRow>
                  <TableCell colSpan={6} className="text-center text-muted-foreground py-8">
                    {lang === "ar" ? "لا توجد تحويلات" : "No transfers"}
                  </TableCell>
                </TableRow>
              ) : (
                list.map((r) => (
                  <TableRow key={r.id}>
                    ...
                    <TableCell className="text-end font-mono">
                      {r.stock_transfer_items?.length ?? 0}
                    </TableCell>
                  </TableRow>
                ))
              )}
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="max-w-3xl">
        <DialogHeader>
          <DialogTitle>{lang === "ar" ? "تحويل مخزون" : "Stock transfer"}</DialogTitle>
        </DialogHeader>
        <div className="grid gap-3">
          <div className="grid grid-cols-2 gap-3">
            ...
            <div className="grid gap-1.5">
              <Label>{lang === "ar" ? "إلى مستودع" : "To warehouse"}</Label>
