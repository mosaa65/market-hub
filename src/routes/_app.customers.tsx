...
import {
  ...
  CreditCard, Calendar, MapPin, Loader2,
} from "lucide-react";
...
function CustomersPage() {
  ...
  const goDebts = useCallback((c: Customer) => {
    navigate({ to: "/debts", search: { customerId: c.id } as any });
  }, [navigate]);
  const goStatement = useCallback((c: Customer) => {
    navigate({ to: "/account-statement", search: { customerId: c.id } as any });
  }, [navigate]);
  const goPOS = useCallback((c: Customer) => {
    navigate({ to: "/pos", search: { customerId: c.id } as any });
  }, [navigate]);
  const goSales = useCallback((c: Customer) => {
    navigate({ to: "/sales", search: { customerId: c.id } as any });
  }, [navigate]);

  const printCustomerStatement = useCallback((c: Customer) => {
    const rtl = lang === "ar";
    const esc = (value: string | number | null | undefined) => String(value ?? "—").replace(/[&<>"']/g, ch => ({ "&": "&amp;", "<": "&lt;", ">": "&gt;", '"': "&quot;", "'": "&#39;" })[ch]!);
    const date = (value: string) => new Date(value).toLocaleDateString(rtl ? "ar-SA" : "en-GB");
    const activityRows = activity.map(a => `<tr><td>${esc(date(a.date))}</td><td>${esc(a.label)}</td><td class="${a.kind === "sale" ? "balance" : "paid"}">${a.kind === "sale" ? "+" : "−"}${esc(money(a.amount))}</td></tr>`).join("");
    const popup = window.open("", "_blank", "noopener,noreferrer");
    if (!popup) return;
    popup.document.write(`<!doctype html><html lang="${rtl ? "ar" : "en"}" dir="${rtl ? "rtl" : "ltr"}"><head><meta charset="utf-8"><title>${rtl ? "كشف حساب" : "Account statement"} — ${esc(c.name)}</title><style>
    ...
    function handler(e: KeyboardEvent) {
      ...
      switch (e.key) {
        case "F1":
          ...
          break;
        case "F2":
          if (active) { e.preventDefault(); setEdit(active); }
          else toast.info(lang === "ar" ? "مرر المؤشر فوق عميل للتعديل (F2)" : "Hover over a customer to edit (F2)");
          break;
        case "F3":
          if (active) { e.preventDefault(); goPayment(active); }
          else toast.info(lang === "ar" ? "مرر المؤشر فوق عميل للتحصيل (F3)" : "Hover over a customer to collect payment (F3)");
          break;
        case "F4":
          if (active) { e.preventDefault(); void remove(active.id); }
          else toast.info(lang === "ar" ? "مرر المؤشر فوق عميل للحذف (F4)" : "Hover over a customer to delete (F4)");
          break;
        case "F5":
          if (active) { e.preventDefault(); goStatement(active); }
          else toast.info(lang === "ar" ? "مرر المؤشر فوق عميل لكشف الحساب (F5)" : "Hover over a customer for statement (F5)");
          break;
        case "F6":
          if (active) {
            ...
            toast.info(lang === "ar" ? "مرر المؤشر فوق عميل لعرض الدين (F6)" : "Hover over a customer to view debt (F6)");
          }
          break;
        case "Escape":
          setMenuOpen(null);
  ...
  const detailStats = useMemo(() => {
    ...
    return { invoiceItems, paymentItems, totalPurchases, totalPayments, invoiceCount: invoiceItems.length, paymentCount: paymentItems.length, lastActivity };
  }, [selected, activity]);
      ...
      <div className="mb-4 flex items-center justify-between rounded-xl border border-border/60 bg-surface/60 backdrop-blur-sm px-4 py-2.5">
        <div className="flex items-center gap-6 text-xs">
          <div className="flex items-center gap-2">
            ...
            <span className="text-muted-foreground">{lang === "ar" ? "المبلغ عليهم:" : "Owed by them:"}</span>
            <span className="font-mono font-semibold text-amber-500">{money(totalOwed)}</span>
          </div>
          <div className="h-4 w-px bg-border/60" />
          <div className="flex items-center gap-2">
            <span className="inline-flex h-2 w-2 rounded-full bg-emerald-500" />
            <span className="text-muted-foreground">{lang === "ar" ? "المبلغ لهم:" : "Owed to them:"}</span>
          ...
          <table className="w-full text-sm">
            <thead className="text-xs uppercase tracking-wide text-muted-foreground">
              <tr className="border-b border-border">
                ...
                <th className="px-3 py-2 text-end font-medium">{lang === "ar" ? "الإجراءات" : "Actions"}</th>
              </tr>
            </thead>
            <tbody>
                ...
                >
                  <td className="px-3 py-2.5">
                    ...
                    {Number(r.credit_limit) > 0 && (
                      <div className="text-[11px] text-rose-500 font-mono mt-0.5">
                        {lang === "ar" ? "حد الائتمان:" : "Credit:"} {money(Number(r.credit_limit))}
                      </div>
                    )}
                  </td>
                  ...
                  <td className="px-3 py-2.5 text-end">
                    <div className="flex items-center justify-end gap-1.5 relative">
                      {/* Edit with F2 below */}
                      <div className="flex flex-col items-center">
                        <button onClick={e => { e.stopPropagation(); setEdit(r); }} title={`${t("common.edit")} (F2)`}
                        ...
                        <span className="text-[9px] text-muted-foreground/50 font-mono leading-none mt-0.5">F2</span>
                      </div>

                      {/* Delete with F4 below */}
                      <div className="flex flex-col items-center">
                        <button onClick={e => { e.stopPropagation(); void remove(r.id); }} title={`${t("common.delete")} (F4)`}
                        ...
                        <span className="text-[9px] text-muted-foreground/50 font-mono leading-none mt-0.5">F4</span>
                      </div>
                      ...
                      <div className="flex flex-col items-center">
                        ...
                        <span className="text-[9px] text-muted-foreground/50 font-mono leading-none mt-0.5">⋯</span>
                      </div>
                        ...
                        >
                          ...
                          {Number(r.loyalty_points) > 0 && (
                            <QuickActionItem
                              ...
                              onClick={() => { setMenuOpen(null); navigate({ to: "/loyalty" }); }}
                            />
                          )}
                          <QuickActionItem
                            icon={<Receipt className="h-3.5 w-3.5" />}
                            label={lang === "ar" ? "فواتير العميل" : "Customer invoices"}
                            onClick={() => { setMenuOpen(null); goSales(r); }}
                          />
                          <div className="my-1 border-t border-border/40" />
                          <QuickActionItem
                            ...
                            shortcut="F2"
                            onClick={() => { setMenuOpen(null); setEdit(r); }}
                          />
                          <QuickActionItem
                            ...
                            shortcut="F4"
                            tone="red"
                            onClick={() => { setMenuOpen(null); void remove(r.id); }}
                          />
                        </div>
                      )}
                    </div>
        ...
        <div className="fixed inset-0 z-50 grid place-items-center bg-background/80 backdrop-blur-sm p-4">
          <div className="panel-elevated w-full max-w-md p-6">
            ...
            <div className="space-y-3">
              <InputField label={`${t("common.name")} *`} value={edit.name ?? ""} onChange={v => setEdit({ ...edit, name: v })} />
              <div className="grid grid-cols-2 gap-3">
                <InputField label={t("common.phone")} value={edit.phone ?? ""} onChange={v => setEdit({ ...edit, phone: v })} />
                <InputField label={t("common.email")} value={edit.email ?? ""} onChange={v => setEdit({ ...edit, email: v })} type="email" />
              </div>
              <InputField label={t("common.address")} value={edit.address ?? ""} onChange={v => setEdit({ ...edit, address: v })} />
              <div>
                <InputField label={t("customers.credit_limit")} value={String(edit.credit_limit ?? 0)} onChange={v => setEdit({ ...edit, credit_limit: Number(v) as any })} type="number" />
            ...
            <div className="mt-5 flex justify-end gap-2">
              ...
              <button onClick={save} disabled={saving} className="flex h-9 items-center justify-center gap-1.5 rounded-md bg-primary px-4 text-sm font-medium text-primary-foreground hover:opacity-90 disabled:opacity-50 disabled:pointer-events-none transition">
        ...
        <div className="fixed inset-0 z-50 grid place-items-center bg-background/80 p-4 backdrop-blur-sm">
          <div className="panel-elevated w-full max-w-2xl p-6 max-h-[90vh] flex flex-col">
            ...
            <div className="mb-4 flex items-start justify-between gap-3">
              <div>
                ...
                <div className="flex flex-wrap items-center gap-2 mt-1 text-xs text-muted-foreground">
                  ...
                  {selected.address && <><span>·</span><span className="flex items-center gap-0.5"><MapPin className="h-3 w-3" />{selected.address}</span></>}
                </div>
                <div className="flex items-center gap-2 mt-1.5">
                  <span className={`inline-flex rounded-full border px-2 py-0.5 text-[10px] ${selected.is_active ? "border-emerald-500/20 bg-emerald-500/10 text-emerald-400" : "border-border bg-muted text-muted-foreground"}`}>
                    {selected.is_active ? t("common.active") : t("common.inactive")}
                  </span>
                  {Number(selected.credit_limit) > 0 && Number(selected.balance) > Number(selected.credit_limit) && (
                    <span className="inline-flex items-center gap-1 rounded-full border border-rose-500/30 bg-rose-500/10 px-2 py-0.5 text-[10px] text-rose-500">
                      <AlertTriangle className="h-3 w-3" /> {lang === "ar" ? "تجاوز حد الائتمان" : "Over credit limit"}
                    </span>
                  )}
                  {Number(selected.loyalty_points) > 0 && (
                    <span className="inline-flex items-center gap-1 rounded-full border border-amber-500/20 bg-amber-500/10 px-2 py-0.5 text-[10px] text-amber-500">
                      <Star className="h-3 w-3" /> {selected.loyalty_points} {lang === "ar" ? "نقطة" : "pts"}
                    </span>
                  )}
                </div>
              </div>
            ...
            <div className="mb-4 grid grid-cols-2 sm:grid-cols-4 gap-2">
              <MiniStat label={lang === "ar" ? "الرصيد المستحق" : "Balance"} value={money(Number(selected.balance))} color="text-primary" />
              <MiniStat label={lang === "ar" ? "حد الائتمان" : "Credit limit"} value={money(Number(selected.credit_limit))} />
              <MiniStat label={lang === "ar" ? "إجمالي الفواتير" : "Total invoices"} value={String(detailStats.invoiceCount)} />
              <MiniStat label={lang === "ar" ? "إجمالي المدفوع" : "Total paid"} value={money(detailStats.totalPayments)} color="text-emerald-500" />
            </div>

            {/* Quick action buttons */}
            <div className="mb-4 flex flex-wrap gap-2">
              <DetailActionBtn icon={<Wallet className="h-3.5 w-3.5" />} label={lang === "ar" ? "تحصيل دفعة" : "Collect"} onClick={() => goPayment(selected)} />
              {Number(selected.balance) > 0 && (
                <DetailActionBtn icon={<AlertTriangle className="h-3.5 w-3.5" />} label={lang === "ar" ? "الدين" : "Debt"} onClick={() => goDebts(selected)} />
              )}
              <DetailActionBtn icon={<FileText className="h-3.5 w-3.5" />} label={lang === "ar" ? "كشف حساب" : "Statement"} onClick={() => goStatement(selected)} />
              <DetailActionBtn icon={<ShoppingCart className="h-3.5 w-3.5" />} label={lang === "ar" ? "فاتورة بيع" : "New sale"} onClick={() => goPOS(selected)} />
              <DetailActionBtn icon={<Printer className="h-3.5 w-3.5" />} label={lang === "ar" ? "طباعة كشف" : "Print"} onClick={() => printCustomerStatement(selected)} />
            </div>
            ...
            <div className="flex items-center gap-1 mb-3 border-b border-border pb-1">
              {(["all", "invoices", "payments"] as const).map(tab => (
                <button key={tab} onClick={() => setDetailTab(tab)}
                  ...
                  {tab === "all" ? (lang === "ar" ? "الكل" : "All") + ` (${activity.length})`
                   : tab === "invoices" ? (lang === "ar" ? "فواتير" : "Invoices") + ` (${detailStats.invoiceCount})`
            ...
            <div className="flex-1 min-h-0 overflow-y-auto space-y-2">
              {(() => {
                const items = detailTab === "all" ? activity
                  : detailTab === "invoices" ? detailStats.invoiceItems
                  : detailStats.paymentItems;
                return items.length === 0
                  ? <p className="py-6 text-center text-sm text-muted-foreground">{lang === "ar" ? "لا توجد حركات لهذا العميل" : "No activity for this customer"}</p>
                  : items.map((item, index) => (
                    <div key={`${item.label}-${index}`} className="flex items-center justify-between rounded-lg border border-border/60 p-2.5 text-sm hover:bg-surface-2/40 transition">
                      <div>
                        <div className="flex items-center gap-1.5">
                          {item.kind === "sale" ? <Receipt className="h-3 w-3 text-rose-500" /> : <CreditCard className="h-3 w-3 text-emerald-500" />}
                      ...
                      <span className={`font-mono font-semibold ${item.kind === "sale" ? "text-rose-500" : "text-emerald-500"}`}>
            ...
            {detailStats.lastActivity && (
              <div className="mt-3 pt-2 border-t border-border/40 text-[11px] text-muted-foreground flex justify-between">
                <span>{lang === "ar" ? "آخر حركة:" : "Last activity:"} {new Date(detailStats.lastActivity).toLocaleDateString()}</span>
                <span>{lang === "ar" ? "تاريخ الإنشاء:" : "Created:"} {new Date(selected.created_at).toLocaleDateString()}</span>
              </div>
            )}
          </div>
        </div>
...
function QuickActionItem({ icon, label, shortcut, badge, tone, onClick }: {
  icon: React.ReactNode; label: string; shortcut?: string; badge?: string; tone?: "red" | "amber"; onClick: () => void;
}) {
  const color = tone === "red" ? "text-destructive hover:bg-destructive/10"
    : tone === "amber" ? "text-amber-500 hover:bg-amber-500/10"
  ...
  return (
    <button onClick={onClick}
      ...
      {badge && <span className="rounded-full bg-amber-500/15 px-1.5 py-0.5 text-[9px] font-mono text-amber-500">{badge}</span>}
      {shortcut && <kbd className="rounded border border-border/40 bg-background/60 px-1.5 py-0.5 text-[9px] font-mono text-muted-foreground">{shortcut}</kbd>}
    </button>
  );
}
...
function MiniStat({ label, value, color }: { label: string; value: string; color?: string }) {
  return (
    <div className="rounded-lg bg-surface-2/60 p-2.5 text-center">
      ...
      <div className={`font-mono text-sm font-bold mt-0.5 ${color ?? "text-foreground"}`}>{value}</div>
    </div>
  );
}

function DetailActionBtn({ icon, label, onClick }: { icon: React.ReactNode; label: string; onClick: () => void }) {
  return (
    <button onClick={onClick}
