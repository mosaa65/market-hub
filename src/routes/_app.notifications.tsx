    ...
    (async () => {
      ...
      (prods.data ?? []).forEach((p: any) => {
        ...
        if (q <= 0)
          out.push({
            kind: "out",
            title: name,
            sub: lang === "ar" ? "نفد المخزون" : "Out of stock",
            severity: "danger",
          });
        else if (q <= min && min > 0)
          out.push({
            kind: "low",
            title: name,
            sub: lang === "ar" ? `الكمية ${q} ≤ الحد ${min}` : `${q} on hand ≤ min ${min}`,
            severity: "warn",
          });
      });
      ...
      (custs.data ?? []).forEach((c: any) => {
        ...
        out.push({
          kind: "debt_c",
          title: c.name,
          sub: lang === "ar" ? "ذمم مدينة" : "Outstanding receivable",
      ...
      (supps.data ?? []).forEach((s: any) => {
        out.push({
          kind: "debt_s",
          title: s.name,
          sub: lang === "ar" ? "ذمم دائنة" : "Outstanding payable",
  ...
  return (
    <>
      <PageHeader
        ...
        subtitle={
          lang === "ar"
            ? "تنبيهات المخزون والذمم في الوقت الحقيقي"
            : "Live stock & receivables alerts"
        }
      />
      <div className="grid grid-cols-1 md:grid-cols-3 gap-3 mb-6">
        <Summary
          label={lang === "ar" ? "حرجة" : "Critical"}
          count={groups.danger.length}
          color="text-rose-500"
        />
        <Summary
          label={lang === "ar" ? "تحذيرات" : "Warnings"}
          count={groups.warn.length}
          color="text-amber-500"
        />
        <Summary
          label={lang === "ar" ? "إخطارات" : "Info"}
          count={groups.info.length}
          color="text-sky-500"
        />
      </div>
      <Card>
