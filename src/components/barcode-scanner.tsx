...
export function BarcodeScanner({ open, onClose, onDetected, continuous = false }: Props) {
  ...
  const stopScanner = useCallback(async (scanner: Html5Qrcode | null = scannerRef.current) => {
    ...
    try {
      ...
      /* Scanner might not have started yet. */
    }
    try {
      await scanner.clear();
    } catch {
      /* Its container may already be unmounted. */
    }
  }, []);
        ...
        {cameras.length > 1 && (
          <div className="mt-3 flex items-center gap-2">
            ...
            <select
              ...
              className="h-9 flex-1 rounded-full border border-border bg-surface px-3 text-xs"
              aria-label={lang === "ar" ? "اختيار الكاميرا" : "Choose camera"}
            >
              {cameras.map((c) => (
            ...
            <button
              onClick={() =>
                setCameraId(
                  cameras[(cameras.findIndex((c) => c.id === cameraId) + 1) % cameras.length].id,
                )
              }
