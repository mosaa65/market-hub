...
## 0. ملاحظة معمارية مهمة — شاشة واحدة

بعد المراجعة، أُعيد تصميم تجربة الاستخدام إلى **شاشة واحدة فقط**:

```text
/account-statement
   ├── شريط الاختيار   (نوع الحساب · الحساب · مجاميع مدين/دائن/رصيد)   ← التصميم الأصلي
   ├── شريط الخيارات   (الفترة · القالب · إظهار المسددة)  ← جديد، مضغوط وقابل للطي
   └── جدول الحركات    (مع سطر افتتاحي وسطر إجماليات)     ← التصميم الأصلي
```

- **لا شاشة منشئ كشف منفصلة**، ولا **مسار مستند مستقل** (أُزيل `_app.statements.$entityType.$entityId`).
- تغيير أي خيار يُعيد الحساب **في نفس المكان** بلا أي انتقال.
- أُزيلت المكوّنات التي لم تبقَ مستخدمة: `statement-document` · `statement-summary-cards` · `statement-transactions-table` · `statement-export-dialog`.
- الأزرار في `debts` / `customers` / `payments` تعود إلى `/account-statement?customerId=...`.
...
| 7 | تجربة الاستخدام: شاشة واحدة | مراجعة المسار | ✅ بلا مسار مستند منفصل |
