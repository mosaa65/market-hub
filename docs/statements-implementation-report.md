...
## 12. إصلاح الدفتر بعد اكتشاف نقص الترحيل
تم تنفيذ إصلاحات قاعدة البيانات على قاعدة المستخدم بعد موافقته الصريحة، مع الحفاظ على البيانات:

| Migration | الإجراء |
| --- | --- |
| `20260916010000_repair_customer_ledger_posting.sql` | ترحيل فروقات قيود المبيعات الناقصة، قيود المرتجعات، وإعادة تعريف `create_sale` ذات 8 معاملات لتضيف قيد `sale` مستقبليًا بشكل idempotent |
| `20260916020000_repair_missing_customer_payment_ledger.sql` | إضافة قيود الدفعات الموجودة التي لم يكن لها قيد `payment` |

### ضمانات السلامة
- لم تُستخدم أي أوامر `UPDATE` أو `DELETE` على العملاء أو الفواتير أو الدفعات أو قيود الدفتر.
- لم تُنشأ أو تُعدَّل أي جداول أو أعمدة أو علاقات أو قيود Schema.
- القيود الجديدة تستخدم `source_key` فريدًا، ولذلك تشغيل الإصلاح مرة أخرى لا يكرر البيانات.
- تم الاحتفاظ بكل القيود القديمة كما هي؛ الإصلاح يضيف القيود الناقصة فقط.
- تمت إعادة تعريف RPC دون حذف توقيع الدالة القديم أو تعديل مستندات المستخدم الموجودة.

### نتيجة التحقق على قاعدة المستخدم
- عدد الفواتير والعملاء والدفعات لم يتغير.
- لا توجد دفعات بلا قيد دفتر بعد الإصلاح.
- لا توجد مجموعات `source_key` مكررة.
- تم تسجيل migration الإصلاحين كـ `applied` في سجل Supabase.
- أصبح كشف العميل يقرأ القيود الموجودة بدل إظهار كشف فارغ.

> **تنبيه:** يوجد migration مستقل لاحق في المستودع غير مطبق على قاعدة المستخدم، وهو خارج نطاق الدفتر ولم يتم تطبيقه ضمن هذا الإصلاح.
